...
def extract_peak(heatmap, max_pool_ks=7, min_score=-5, max_det=100):
    ...
    pooled_heatmap = F.max_pool2d(heatmap[None, None], kernel_size=max_pool_ks, padding=max_pool_ks // 2, stride=1)[0, 0]
    
    possible_det = heatmap - (pooled_heatmap > heatmap).float() * 1e5
    
    if max_det > possible_det.numel():
        max_det = possible_det.numel()
        
        
    top_scores, top_indices = torch.topk(possible_det.view(-1), max_det)
    
    return [(float(s),int(l)  % heatmap.size(1), int(l) // heatmap.size(1)) for s, l in zip(top_scores.cpu(), top_indices.cpu()) if s > min_score]
...
class FocalLoss(nn.Module):
    ...
    def forward(self, inputs, targets):
        ...
        pt = torch.exp(-BCE_loss)  # pt is the probability of being correctly classified
        
        # Adjust alpha based on the targets
